...
#define HyDbg(str, args...)     do {struct timeval tm_tmp; struct tm *ptm = NULL; gettimeofday(&tm_tmp, NULL); tm_tmp.tv_sec += 3600*8; ptm = gmtime(&tm_tmp.tv_sec); \
...
#define HyErr(str, args...)     do {struct timeval tm_tmp; struct tm *ptm = NULL; gettimeofday(&tm_tmp, NULL); tm_tmp.tv_sec += 3600*8; ptm = gmtime(&tm_tmp.tv_sec); \
    printf(COLOR_STR_RED " " TIME_STR " %s>: FUNCTION: %s, LINE: %d, " COLOR_STR_NONE " " str, \
    ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, filename(__FILE__), __FUNCTION__, __LINE__, ## args); } while (0)
...
do { \
    if (!(expect)) \
    { \
        HyErr("assert failed, expect(%s)\n", #expect); \
        return; \
    } \
} while (0)

#define assert_param_return(expect, ret, args...) \
do { \
    if (!(expect)) \
    { \
        HyErr("assert failed, expect \"%s\"\n", #expect); \
